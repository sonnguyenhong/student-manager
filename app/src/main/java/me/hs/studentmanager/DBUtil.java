...
public class DBUtil {
    ...
    public static void update(SQLiteDatabase db, StudentModel studentModel) {
        ...
        try {
            ...
            cv.put("email", studentModel.getEmail());
            cv.put("dob", studentModel.getDob());
            db.update("student", cv, "student_id=?",  new String[]{String.valueOf(studentModel.getStudentID())});
            db.setTransactionSuccessful();
        } catch (Exception ex) {
            ex.printStackTrace();
        } finally {
    ...
    public static void delete(SQLiteDatabase db, StudentModel studentModel) {
        db.beginTransaction();
        try {
            db.delete("student", "student_id=?", new String[]{String.valueOf(studentModel.getStudentID())});
            db.setTransactionSuccessful();
        } catch (Exception ex) {
            ex.printStackTrace();
        } finally {
